...
useEffect 解决了 **"什么时候做"**——渲染完了再做副作用。

但副作用里经常要做网络请求，这引出另一个问题：**"怎么做才不阻塞"**。

| 层面 | 解决什么 | 机制 |
|------|---------|------|
| **React 层**（useEffect） | 什么时候做副作用 | 渲染完了再执行 |
| **JS 层**（async） | 怎么做不冻结页面 | 交给后台，结果回来再处理 |

它们经常搭配：useEffect 控制"渲染完了再做"，里面的 fetch 控制"请求在后台跑"。

<!--
所以 useEffect 和 async 是互补的两层：
- useEffect 是 React 层面：控制"渲染完了再做副作用"
- async 是 JS 层面：控制"网络请求在后台跑，不冻结页面"
你在 useEffect 里发 async 请求，两层各司其职。
-->
...
// ⚠️ 代码执行到这里时，上面的请求还没完成！.then() 只是"预约"了回调，不会阻塞
```
